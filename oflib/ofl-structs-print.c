...
ofl_structs_oxm_tlv_print(FILE *stream, struct ofl_match_tlv *f)
{
	...
	switch (field) {
		...
		case OFPXMT_OFB_IPV6_EXTHDR:
			...
			break;
        /*Modificacion UAH Discovery hybrid topologies, JAH-*/
	    case OFPXMT_OFB_DHT_OPCODE:
            fprintf(stream,"OpCode: \"%"PRIu16"\"",*((uint16_t*)(f->value)));
            break;
        case OFPXMT_OFB_DHT_NUM_DEVICE:
            fprintf(stream,"Number of Devices: \"%"PRIu16"\"",*((uint16_t*)(f->value)));
            break;
        case OFPXMT_OFB_DHT_TYPE_DEVICE:
            fprintf(stream,"Type of Devices: ");
            for (index=0;index<DHT_MAX_ELEMENTS;index++)
            {   aux16=(uint16_t)*(uint16_t*)(f->value+(index*sizeof(uint16_t)));
                if(aux16!=0)
                    fprintf(stream,"\"%"PRIu16"\"",aux32);
            }
            fprintf(stream," }");
            break;
        case OFPXMT_OFB_DHT_MACS:
            fprintf(stream,"MACS: { ");
            for (index=0;index<DHT_MAX_ELEMENTS;index++)
            {   aux64=(uint64_t)*(uint64_t*)(f->value+(index*sizeof(uint64_t)));
                if(aux64!=0)
                    fprintf(stream,"\"%"PRIu64"\"",aux64);
            }
            fprintf(stream," }");
            
            break;
        case OFPXMT_OFB_DHT_IN_PORTS:
            fprintf(stream,"InPortsList: {");
            for (index=0;index<DHT_MAX_ELEMENTS;index++)
            {   aux32=(uint32_t)*(uint32_t*)(f->value+(index*sizeof(uint32_t)));
                if(aux32!=0)
                    fprintf(stream,"\"%"PRIu32"\"",aux32);
            }
            fprintf(stream," }");
            break;
        case OFPXMT_OFB_DHT_OUT_PORTS:
            fprintf(stream,"OutPortsList: {");
            for (index=0;index<DHT_MAX_ELEMENTS;index++)
            {   aux32=(uint32_t)*(uint32_t*)(f->value+(index*sizeof(uint32_t)));
                if(aux32!=0)
                    fprintf(stream,"\"%"PRIu32"\"",aux32);
