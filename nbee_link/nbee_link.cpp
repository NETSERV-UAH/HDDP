...
/* Found a NetPDL field usable on matching */    
    ...
    if (field->Mask != NULL)
    {
        ...
        else if(header==OXM_OF_DHT_OPCODE || header == OXM_OF_DHT_NUM_DEVICE || header == OXM_OF_DHT_TYPE_DEVICE)
        {
            uint16_t m_value =  *((uint16_t*)((uint8_t*)pktin->data + field->Position));
            m_value =  ntohs(m_value);
            ofl_structs_match_put16(pktout, header, m_value);
        }
        else if(header==OXM_OF_DHT_MACS)
        {
            uint64_t * m_value =  ((uint64_t*) ((uint8_t*)pktin->data + field->Position));
            ofl_structs_match_put_macs(pktout, header, m_value);
        }
        else if (header==OXM_OF_DHT_IN_PORTS || header==OXM_OF_DHT_OUT_PORTS)
        {
            uint32_t * m_value =  ((uint32_t*) ((uint8_t*)pktin->data + field->Position));
            ofl_structs_match_put_port(pktout, header, m_value);
        }
    ...
    else
    {
        switch (field->Size){
            ...
            /*Modificacion UAH Discovery hybrid topologies, JAH-*/
            case (sizeof(uint32_t)*DHT_MAX_ELEMENTS):
            {
                uint32_t * m_value =  ((uint32_t*) ((uint8_t*)pktin->data + field->Position));
                ofl_structs_match_put_port(pktout, header, m_value);//Es la misma funcion que para out_ports, ya que es el mismo tamanyo
                break;
            }
            case  (sizeof(uint64_t)*DHT_MAX_ELEMENTS):
            {
                uint64_t * m_value =  ((uint64_t*) ((uint8_t*)pktin->data + field->Position));
                ofl_structs_match_put_macs(pktout, header, m_value);
                break;
...
extern "C" int nblink_packet_parse(struct ofpbuf * pktin,  struct ofl_match * pktout, struct protocols_std * pkt_proto)
{
    ...
    while (proto!= NULL)
    {
        /* Getting first field of the protocol  */
            ...
            /*Modificacion UAH Discovery hybrid topologies, JAH-*/
            else if (protocol_Name.compare("DHT")==0 && pkt_proto->dht==NULL)
            {
                pkt_proto->dht= (struct dht_header *) ((uint8_t*) pktin->data + proto->Position);
                PDMLReader->GetPDMLField(proto->Name, (char*) "opcode", proto->FirstField, &field);
                nblink_extract_proto_fields(pktin, field, pktout, OXM_OF_DHT_OPCODE);
                PDMLReader->GetPDMLField(proto->Name, (char*) "num_devices", proto->FirstField, &field);
                nblink_extract_proto_fields(pktin, field, pktout, OXM_OF_DHT_NUM_DEVICE);
                PDMLReader->GetPDMLField(proto->Name, (char*) "type_devices", proto->FirstField, &field);
                nblink_extract_proto_fields(pktin, field, pktout, OXM_OF_DHT_TYPE_DEVICE);
                PDMLReader->GetPDMLField(proto->Name, (char*) "macs", proto->FirstField, &field);
                nblink_extract_proto_fields(pktin, field, pktout, OXM_OF_DHT_MACS);
                PDMLReader->GetPDMLField(proto->Name, (char*) "in_ports", proto->FirstField, &field);
                nblink_extract_proto_fields(pktin, field, pktout, OXM_OF_DHT_IN_PORTS);
                PDMLReader->GetPDMLField(proto->Name, (char*) "out_ports", proto->FirstField, &field);
                nblink_extract_proto_fields(pktin, field, pktout, OXM_OF_DHT_OUT_PORTS);
            }
            /*Fin Modificacion UAH Discovery hybrid topologies, JAH-*/
