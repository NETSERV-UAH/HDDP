...
public class DHTproviderlink implements LinkProvider{
    ...
    public void linkVanished(String srcname, String dstname, LinkProviderService linkProviderService) {
        ...
        if ((!srcId.contains("sw") && !srcId.contains("of")) ||
                (!dstId.contains("sw") && !dstId.contains("of"))){
            /** Le comunicaciones que es bidireccional y no es durable */
            link_config = new BasicLinkConfig(linkKey1)
                    .isBidirectional(true).isDurable(false);
        }
        else{
    ...
    public boolean linkbewteendevices(DeviceService deviceService, LinkProviderService linkProviderService,
        ...
        LinkDescription ld1 = new DefaultLinkDescription(linkKey1.src(), linkKey1.dst(), Link.Type.DIRECT, sa);
        LinkDescription ld2 = new DefaultLinkDescription(linkKey2.src(), linkKey2.dst(), Link.Type.DIRECT, sa);

        log.debug("LD create OK {} and {}", ld1, ld2);

        /** Genero el linkkey entre ambos equipos */


        if (configuredLinks.contains(linkKey1) || configuredLinks.contains(linkKey2)) {
            log.debug("Found configured link {} or {}", linkKey1, linkKey2);
            try {
                linkVanished(ld1,linkProviderService);
                linkVanished(ld2,linkProviderService);
                log.debug("Remove links OK {} and {}", linkKey1, linkKey2);
        ...
        log.debug("Found link that was not in the configuration {} and {}", linkKey1, linkKey2);
        try{
